...
public class AuthController {
    ...
    @Operation(
            ...
            description = "Client data from token"
    )
    @ApiResponse(
            responseCode = "200",
    ...
    @Operation(
            summary = "Sign in",
            description = "Login via phone and password"
    )
    @ApiResponses({
            @ApiResponse(
                    responseCode = "200",
    ...
    @Operation(
            summary = "Sign up",
            description = "Create a new user → OTP will be sent"
    )
    @ApiResponse(
            responseCode = "200",
            description = "OTP code will be returned (for example: 123456)"
    )
    ...
    @Operation(
            summary = "Confirm with OTP code",
            description = "Account activation via OTP"
    )
    @ApiResponse(
            responseCode = "200",
            description = "Account activated - (UserData + JWT token)"
    )
    @PostMapping("/verify")
    ...
    @Operation(
            summary = "Reset password via OTP code",
            description = "Allows the user to reset their password using a verified OTP code."
    )
    @ApiResponse(
            responseCode = "200",
    ...
    @Operation(
            summary = "Change registered phone number",
            description = "Allows the user to change their registered phone number after OTP verification."
    )
    @ApiResponse(
            responseCode = "200",
            description = "Phone number successfully changed."
    )
    @PostMapping("/change-phone-number")
    public ResponseEntity<?> changeClientPhoneNumber() {
        return ResponseEntity.ok().build();
    }
